...
function validarProducto(body, { partial = false } = {}) {
    ...
    if (stock_minimo !== undefined && (!Number.isInteger(+stock_minimo) || +stock_minimo < 0)) errors.push('Stock mínimo debe ser entero ≥ 0');
    if (precio !== undefined && (isNaN(+precio) || +precio < 0)) errors.push('Precio debe ser ≥ 0');
    if (categoria_id !== undefined && categoria_id !== null && !Number.isInteger(+categoria_id)) errors.push('Categoría no válida');
    return errors;
}
...
router.get('/', async (req, res) => {
    ...
    if (stockBajo) { where.push('(p.stock - p.stock_reservado) <= p.stock_minimo'); }
    ...
    const [[{ total }]] = await pool.query(
        `SELECT COUNT(*) AS total FROM productos p ${whereSql}`, params
    );
...
router.post('/', authRequired, requireRole('admin','operario'), async (req, res) => {
    ...
    const { sku, nombre, descripcion, categoria_id, ubicacion, stock = 0, stock_minimo = 5, precio = 0, imagen_url } = req.body;
    try {
        const [r] = await pool.query(
            `INSERT INTO productos (sku, nombre, descripcion, categoria_id, ubicacion, stock, stock_minimo, precio, imagen_url)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [sku, nombre, descripcion || null, categoria_id || null, ubicacion, stock, stock_minimo, precio, imagen_url || null]
        );
        if (stock > 0) {
            await pool.query(
...
router.put('/:id', authRequired, requireRole('admin','operario'), async (req, res) => {
    ...
    if (errs.length) return res.status(400).json({ error: errs.join('; '), detalles: errs });
    const { sku, nombre, descripcion, categoria_id, ubicacion, stock, stock_minimo, precio, imagen_url, activo } = req.body;
    ...
    try {
        await pool.query(
            `UPDATE productos SET
             ...
             WHERE id = ?`,
            [sku, nombre, descripcion, categoria_id, ubicacion, stock, stock_minimo, precio, imagen_url, activo, req.params.id]
        );
